...
function getTransactionBadge(transaction: Expense, memberId: string) {
  if (transaction.kind === "settlement") {
    return transaction.paidById === memberId ? "還款" : "收款";
  }
  return transaction.type === "paid" ? "付款" : "分攤";
}

function getTransactionDescription(transaction: Expense) {
  if (transaction.kind === "settlement") {
    const recipient = transaction.participants[0]?.member.name || "Unknown";
    return `${formatDate(transaction.date)} · ${transaction.paidBy.name} 還給 ${recipient}`;
  }
  return `${formatDate(transaction.date)} · 由 ${transaction.paidBy.name} 付款`;
}
      ...
      <Card className="p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right">
            <p className="text-sm font-semibold text-slate-500">目前餘額</p>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-4">
          <div className="mb-4 flex items-center gap-2">
            <UserRound size={18} />
            <h3 className="font-bold text-slate-950">與其他成員的關係</h3>
          </div>
          {data.debtRelationships.length === 0 ? (
            <p className="text-sm text-slate-500">目前沒有未平衡的往來。</p>
          ) : (
        ...
        <Card className="p-4">
          <div className="mb-4 flex items-center gap-2">
            <ReceiptText size={18} />
            <h3 className="font-bold text-slate-950">交易摘要</h3>
          </div>
          <p className="text-sm text-slate-500">共 {data.transactions.length} 筆付款、分攤或還款紀錄。</p>
        </Card>
      </div>

      {data.transactions.length === 0 ? (
        <EmptyState title="尚無交易" description="這位成員還沒有付款、分攤或還款紀錄。" />
      ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <div className="mb-2 flex min-w-0 flex-wrap items-center gap-2">
                        ...
                        <Badge tone={isSettlement ? "blue" : transaction.type === "paid" ? "green" : "slate"}>
                          {getTransactionBadge(transaction, memberId)}
                        </Badge>
                      </div>
                      <p className="text-sm text-slate-500">{getTransactionDescription(transaction)}</p>
                    </div>
                    <div className="flex shrink-0 items-start gap-2">
                      <p className="max-w-full break-words font-bold text-slate-950">{formatCurrency(transaction.amount)}</p>
                      <ChevronDown
                        size={18}
                          ...
                          >
                            <div className="flex min-w-0 items-center gap-2">
                              ...
                              <span className="truncate text-sm font-semibold text-slate-700">
                                {isSettlement ? `收款方：${participant.member.name}` : participant.member.name}
                              </span>
                              {isCurrentMember && <Badge tone="blue">此成員</Badge>}
                            </div>
                            <span className="max-w-full shrink-0 break-words text-sm font-bold tabular-nums text-slate-950">
